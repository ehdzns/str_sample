...
Company = READ_COMPANY('../data/sample_4월_데일리 리포트_fin.xlsx') # 수정: 회사 설정 필요 
...
with compare_container:
    ...
    y=alt.Y("values:Q", axis=alt.Axis(title="변화량"), scale=alt.Scale(domain=(min_value, max_value))),
    text='values:Q',
    color=alt.condition(
        alt.datum.values > 0,
...
with comment_container:
    ...
    if api_input:
        ...
        try:
            ...
            api_valid = True
        except Exception as e:  # 여기서 Exception은 실제 발생하는 예외 타입으로 대체해야 합니다.
            api_valid = False
            st.error("API 키가 올바르지 않습니다. 다시 확인해주세요.")

    if st.button('코멘트 생성', key='generate') and api_valid:
        try:
        ...
        except Exception as e:  # 예외 처리: 코멘트 생성 중 에러 발생 시
